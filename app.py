...
- KFTA 표준 변환 없음
        ...
        with st.expander("ℹ️ 사용 방법"):
            st.markdown("""
            1. **파일 업로드**: 여러 엑셀 파일을 드래그하거나 선택하세요
            2. **설정 확인**: AI 모드, 유사도 임계값, 출력 형식을 선택하세요
            3. **통합 실행**: 데이터 분석 탭에서 통합을 시작하세요
            4. **결과 확인**: 통합된 데이터를 미리보고 검증하세요
            5. **다운로드**: Excel 또는 CSV 형식으로 다운로드하세요
            """)

        with st.expander("✨ 주요 기능"):
            st.markdown("""
            - **🤖 AI 컬럼 매핑**: 유사한 컬럼명을 지능적으로 자동 통일
            - **🎓 자가 학습 시스템**: 사용할수록 교육청 매핑 정확도 향상
            - **🏫 강원도 학교 DB**: 100+ 학교 정보로 자동 매칭
            - **📋 KFTA 표준 변환**: 강원교총 12개 표준 컬럼 자동 변환
            - **🔍 실시간 미리보기**: 통합 결과를 즉시 확인
            - **📊 다중 형식 지원**: Excel, CSV 입출력 및 다운로드
            """)
        ...
        with st.expander("🎓 자가 학습 시스템"):
            st.markdown("""
            **사용할수록 똑똑해지는 AI**

            이 앱은 데이터를 처리할 때마다 자동으로 학습하여 점점 더 정확해집니다.

            #### 💾 자동 학습 방식

            1. **성공한 매핑 저장**
               - 학교명 → 교육청 매핑이 성공하면 자동 저장
               - 다음번에는 즉시 정확하게 매칭
               - 저장 위치: `learned_school_mappings.json`

            2. **실패 기록 추적**
               - 매칭 실패한 학교는 별도 로그에 기록
               - 관리자가 수동으로 DB에 추가 가능
               - 기록 위치: `failed_mappings.log`

            3. **6단계 지능형 매칭**
               - ✅ 1단계: 학습된 매핑 우선 검색
               - ✅ 2단계: 강원도 전체 학교 DB (100+ 학교)
               - ✅ 3단계: 중·고교 지역 키워드 매칭
               - ✅ 4단계: 학교명 내 지역명 추출
               - ✅ 5단계: 행 전체에서 힌트 검색 (교육청, 지역명)
               - ✅ 6단계: 학교명 앞 2-3글자 지역명 분석

            #### 📈 학습 효과

            - **첫 사용**: 기본 DB로 약 70-80% 매칭
            - **10회 사용 후**: 학습 데이터 축적으로 85-90% 매칭
            - **100회 사용 후**: 조직 특화 DB 완성으로 95%+ 매칭

            💡 **팁**: 처음에 매칭 안 된 학교도 반복 사용하면 자동으로 학습됩니다!
            """)
    ...
    with tab2:
        ...
        if not st.session_state.uploaded_files_data:
            st.info("👈 먼저 **파일 업로드** 탭에서 파일을 업로드해주세요")
        else:
            # 준비 상태 안내
            st.success(f"✅ {len(st.session_state.uploaded_files_data)}개 파일이 준비되었습니다. 아래 버튼을 클릭하여 통합을 시작하세요.")
    ...
    with tab3:
        ...
        if st.session_state.unified_df is None:
            st.info("👈 먼저 **데이터 분석** 탭에서 통합을 실행해주세요")
        else:
    ...
    with footer_col1:
        st.caption(f"📌 버전 {__version__} ({__release_date__})")
    with footer_col2:
        st.caption("🔗 [GitHub Repository](https://github.com/caleblee2050/kfta_excel)")
    with footer_col3:
